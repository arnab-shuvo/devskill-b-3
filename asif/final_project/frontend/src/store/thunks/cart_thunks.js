...
export const get_carts = (navigate) => (dispatch, getState) => {
  const { user_reducer } = getState();
  axios
    .get(`http://localhost:8000/carts?customer_id=${user_reducer.user?.id}`)
    .then((response) =>
      response.data.carts.length === 0
        ? cogoToast.warn("you didnot added carts yet") && navigate("/")
        : dispatch({
            type: add_carts,
